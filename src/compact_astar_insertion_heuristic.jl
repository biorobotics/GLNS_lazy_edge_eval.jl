...
function astar_insertion!(sets_to_insert::Vector{Int64}, dist::AbstractArray{Int64, 2}, sets::Vector{Vector{Int64}}, membership::Vector{Int64}, inf_val::Int64, stop_time::Float64, vd_info::VDInfo, partial_tour::Vector{Int64}, known_feas_tour::Vector{Int64}, known_feas_f_val::Int64)
  ...
  while length(open_list) != 0 && goal_node.f_val > peek(open_list).first.f_val
    if peek(open_list).first.f_val >= known_feas_f_val
      return known_feas_tour
    end
    ...
    for (removed_set_idx, node_idx) in zip(neighbor_removed_sets, neighbors)
      ...
      h_prev = h_val
      for (removed_set_idx2, set_idx2) in enumerate(sets_to_insert)
        if removed_set_idx == removed_set_idx2
          continue
        end
        if pop.visited_removed_sets[removed_set_idx2]
          continue
        end
        hmin = inf_val
        for node_idx2 in sets[set_idx2]
          hmin = min(hmin, dist[node_idx, node_idx2])
        end
        h_val += hmin
      end
      # h_val = 0
