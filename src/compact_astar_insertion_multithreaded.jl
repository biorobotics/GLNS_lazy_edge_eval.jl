...
struct VDInfo
  before::Array{Bool, 2}
  neighbor_nodes::Vector{Union{Nothing,VDNode}}
...
function VDInfo(dist::AbstractArray{Int64, 2}, sets::Vector{Vector{Int64}}, membership::Vector{Int64}, inf_val::Int64, max_removals::Int64)
  lengths = [length(set) for set in sets]
  partialsort!(lengths, max_removals; rev=true)
  max_tasks = sum(lengths[1:max_removals]) + 1
  vd_info = VDInfo(ones(Bool, length(membership), length(sets)), Vector{Union{Nothing,VDNode}}(undef, max_tasks), Vector{Tuple{Int64,Int64}}(undef, max_tasks), zeros(11)...)
  if length(sets) == 0
    return vd_info
  end

  for task_idx=1:max_tasks
    vd_info.neighbor_nodes[task_idx] = nothing
  end
  ...
  while length(open_list) != 0 && goal_node.f_val > peek(open_list).first.f_val
    ...
    for (removed_set_idx, set_idx) in zip(this_removed_sets, this_sets)
      ...
      for node_idx in this_set
        task_idx += 1
        vd_info.tasks[task_idx] = (removed_set_idx, node_idx)
      end
    end
    num_tasks = task_idx

    @threads for task_idx=1:num_tasks
      removed_set_idx, node_idx = vd_info.tasks[task_idx]
      ...
      vd_info.neighbor_nodes[task_idx] = neighbor_node
    end

    for task_idx=1:num_tasks
      if vd_info.neighbor_nodes[task_idx] == nothing
        continue
      end
      neighbor_node = vd_info.neighbor_nodes[task_idx]
      # bt = time_ns()
