...
struct VDInfo
  ...
  before_set_to_set_restricted::Array{Bool, 2}
  before_time::Float64
...
function VDInfo(dist::AbstractArray{Int64, 2}, sets::Vector{Vector{Int64}}, membership::Vector{Int64}, inf_val::Int64)
  vd_info = VDInfo(ones(Bool, length(membership), length(sets)), zeros(Bool, length(sets), length(sets)), zeros(Bool, length(sets), length(sets)), zeros(12)...)
  if length(sets) == 0
    return vd_info
  end
...
function astar_insertion!(sets_to_insert::Vector{Int64}, dist::AbstractArray{Int64, 2}, sets::Vector{Vector{Int64}}, membership::Vector{Int64}, inf_val::Int64, stop_time::Float64, vd_info::VDInfo, partial_tour::Vector{Int64}, known_feas_tour::Vector{Int64})
  ...
  # bt = time_ns()
  # Compute ancestors of nonremoved sets
  prev_set_idx = 0
  for tour_idx=2:length(partial_tour)
    node_idx = partial_tour[tour_idx]
    set_idx = membership[node_idx]
    if prev_set_idx == 0
      vd_info.before_set_to_set_restricted[set_idx, :] = vd_info.before_set_to_set[set_idx, :]
    else
  ...
  for set_idx in sets_to_insert
    # Compute last ancestor
    lastAnc = 1
    for tour_idx=length(partial_tour)-1:2
      if !vd_info.before[partial_tour[tour_idx], set_idx]
        lastAnc = tour_idx
        break
      end
    end
    vd_info.before_set_to_set_restricted[set_idx, :] = vd_info.before_set_to_set[set_idx, :] .|| vd_info.before_set_to_set_restricted[membership[partial_tour[lastAnc]], :]
  end
  ...
  for (removed_set_idx, set_idx) in enumerate(sets_to_insert)
    Pmin = sum(vd_info.before_set_to_set_restricted[set_idx, :]) + 1
    Pmax = length(sets) - sum(vd_info.before_set_to_set_restricted[:, set_idx])
